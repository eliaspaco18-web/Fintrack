...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  if (fileValue.size <= 0) {
    return apiError({ code: 'VALIDATION_ERROR', message: 'El archivo no puede estar vacio' })
  }
  if (fileValue.size > MAX_FILE_SIZE_BYTES) {
    return apiError({
      code: 'VALIDATION_ERROR',
      message: 'El archivo supera el limite permitido (8 MB)',
    })
  }
  ...
  const attachmentLine = `[adjunto:${fileValue.name}|${path}]`
